...
public class ClusterSet {
  ...
  public void initializeContainers(int numberOfPoints, int clustersToBeFound,
                                   int numberOfRepInCluster,
                                   double shrinkFactor) {
   ...
   */
  public ClusterSet(ArrayList clusters, int numberOfClusters,
                    int numberOfRepInCluster, double shrinkFactor,
                    HashMap dataPointMap, boolean clusterMerge) {
    numberOfPoints = 0;
   ...
   * CURE parameters
   */
  public ClusterSet(ArrayList dataPoints, int numberOfClusters,
                    int numberOfRepInCluster, double shrinkFactor,
                    HashMap dataPointMap) {
    initializeContainers(dataPoints.size(), numberOfClusters,
                         numberOfRepInCluster, shrinkFactor);
  ...
  */
  ...
  public void buildHeap() {
    ...
    for (int i = 0; i < numberOfPoints; i++) {
      ...
      cluster.distanceFromClosest = points[i].calcDistanceFromPoint(
          nearest); // changed here from indexing to hashmap
   ...
   */
  public void adjustHeap(Cluster newCluster, Cluster oldcluster1,
                         Cluster oldCluster2) {
    ArrayList clusters = new ArrayList();
    ...
    for (int i = 0; i < clusters.size(); i++) {
      ...
      if (!(cluster1.closestCluster == oldcluster1) &&
          !(cluster1.closestCluster == oldCluster2)) {
        heap.add(cluster1);
    ...
    standardDeviationofClusters =
        Math.sqrt(standardDeviationofClusters / clusters.size());
    ...
    Point pj = null;
    if (clusters.size() <= clustersToBeFound + 4 && finalRun == 1) {
      System.out.println("standardDeviationofClusters=" +
                         standardDeviationofClusters);
      for (int i = 0; i < clusters.size(); i++) {
        Cluster cluster1 = (Cluster)clusters.get(i);

        intraDensity +=
            cluster1.intraClusterDensity(standardDeviationofClusters);
        ...
        for (int j = i + 1; j < clusters.size(); j++) {
          ...
          interDensity += (minDistance * cluster1.density(cluster2)) /
                          (cluster1.stdDev() + cluster2.stdDev());
      ...
      System.out.println("Cluster Size=" + clusters.size() + "  clusterSep =" +
                         clusterSep);

      // intraDistance=intraDistance/clusters.size();
      System.out.println("Cluster Size=" + clusters.size() +
                         "  Intra Density=" + intraDensity);
